"""
compute_metrics_AOI.py
计算单个AOI的检测指标（Precision, Recall, F1）
适用于真值文件帧号100-611，检测CSV帧号409-430
"""

import numpy as np
import pandas as pd
import os
import argparse
from sklearn.neighbors import NearestNeighbors
from math import radians, sin, cos, sqrt, asin

def haversine(lon1, lat1, lon2, lat2):
    R = 6371000
    lon1, lat1, lon2, lat2 = map(radians, [lon1, lat1, lon2, lat2])
    dlon = lon2 - lon1
    dlat = lat2 - lat1
    a = sin(dlat/2)**2 + cos(lat1) * cos(lat2) * sin(dlon/2)**2
    c = 2 * asin(sqrt(a))
    return R * c

def filter_stationary_targets(gt_csv, start_frame, end_frame, dist_thresh_m=0.8):
    """加载真值并过滤移动不足的目标，仅保留指定帧范围"""
    df = pd.read_csv(gt_csv)
    # 筛选帧范围
    df = df[(df['FRAME_NUMBER'] >= start_frame) & (df['FRAME_NUMBER'] <= end_frame)]
    if df.empty:
        print(f"警告：在帧范围 {start_frame}-{end_frame} 内没有真值数据")
        return {}
    # 重置索引，使索引连续（避免索引越界）
    df = df.reset_index(drop=True)
    moving_mask = np.zeros(len(df), dtype=bool)
    for obj_id, group in df.groupby('id'):
        group = group.sort_values('FRAME_NUMBER')
        indices = group.index  # 连续索引
        lats = group['LATITUDE'].values
        lons = group['LONGITUDE'].values
        for i in range(len(lats)-1):
            dist = haversine(lons[i], lats[i], lons[i+1], lats[i+1])
            if dist >= dist_thresh_m:
                moving_mask[indices[i]] = True
                moving_mask[indices[i+1]] = True
    df_moving = df[moving_mask]
    gt_by_frame = {}
    for frame, group in df_moving.groupby('FRAME_NUMBER'):
        points = group[['X', 'Y']].values.astype(np.float32)
        gt_by_frame[int(frame)] = points
    return gt_by_frame

def load_detections(detection_folder, frame_range):
    det_by_frame = {}
    total_dets = 0
    for frame in frame_range:
        fname = f"frame{frame:06d}.csv"
        path = os.path.join(detection_folder, fname)
        if not os.path.exists(path):
            det_by_frame[frame] = np.empty((0,2))
        else:
            points = np.loadtxt(path, delimiter=',', dtype=np.float32)
            if points.ndim == 1:
                points = points.reshape(1, -1)
            det_by_frame[frame] = points
            total_dets += points.shape[0]
    print(f"总检测点数: {total_dets}")
    return det_by_frame

def compute_metrics_for_frame(gt_points, det_points, dist_thresh=10.0):
    if len(gt_points) == 0 and len(det_points) == 0:
        return 0, 0, 0
    if len(gt_points) == 0:
        return 0, len(det_points), 0
    if len(det_points) == 0:
        return 0, 0, len(gt_points)
    nbrs = NearestNeighbors(n_neighbors=1).fit(gt_points)
    distances, indices = nbrs.kneighbors(det_points)
    distances = distances.flatten()
    matched_gt = set()
    tp = 0
    for i, dist in enumerate(distances):
        if dist <= dist_thresh:
            gt_idx = indices[i][0]
            if gt_idx not in matched_gt:
                matched_gt.add(gt_idx)
                tp += 1
    fp = len(det_points) - tp
    fn = len(gt_points) - len(matched_gt)
    return tp, fp, fn

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--AOI', type=str, required=True,
                        choices=['01', '02', '03', '34', '40', '41'])
    parser.add_argument('--detection_folder', type=str, required=True,
                        help='检测结果CSV文件夹路径（包含 frame000409.csv 等）')
    parser.add_argument('--gt_csv', type=str, 
                        default='D:/Lkqs/sys/code/WPAFB2009/TrackTruth/TRAIN/20091021_truth_rset1_frames0100-0611.csv',
                        help='真值文件路径')
    parser.add_argument('--start_frame', type=int, default=409,
                        help='起始帧号（检测CSV和真值均使用此帧号）')
    parser.add_argument('--end_frame', type=int, default=430,
                        help='结束帧号')
    parser.add_argument('--dist_thresh', type=float, default=10.0,
                        help='检测匹配距离阈值（像素）')
    parser.add_argument('--move_thresh', type=float, default=0.8,
                        help='移动阈值（米）')
    args = parser.parse_args()

    print(f"评估 AOI {args.AOI}")
    print(f"检测文件夹: {args.detection_folder}")
    print(f"真值文件: {args.gt_csv}")
    print(f"帧范围: {args.start_frame} - {args.end_frame}")

    if not os.path.exists(args.gt_csv):
        print(f"错误：真值文件不存在 {args.gt_csv}")
        return

    # 加载并过滤真值
    print("加载并过滤真值（移除静止目标）...")
    gt_by_frame = filter_stationary_targets(args.gt_csv, args.start_frame, args.end_frame, args.move_thresh)
    if not gt_by_frame:
        print("没有真值数据，请检查帧范围或真值文件")
        return

    frame_range = range(args.start_frame, args.end_frame + 1)

    # 加载检测结果
    print("加载检测结果...")
    det_by_frame = load_detections(args.detection_folder, frame_range)

    total_tp = 0
    total_fp = 0
    total_fn = 0
    frames_processed = 0

    for frame in frame_range:
        gt = gt_by_frame.get(frame, np.empty((0,2)))
        det = det_by_frame.get(frame, np.empty((0,2)))
        tp, fp, fn = compute_metrics_for_frame(gt, det, args.dist_thresh)
        total_tp += tp
        total_fp += fp
        total_fn += fn
        frames_processed += 1
        if frames_processed % 5 == 0:
            print(f"已处理 {frames_processed} 帧")

    precision = total_tp / (total_tp + total_fp) if (total_tp + total_fp) > 0 else 0.0
    recall = total_tp / (total_tp + total_fn) if (total_tp + total_fn) > 0 else 0.0
    f1 = 2 * precision * recall / (precision + recall) if (precision + recall) > 0 else 0.0

    print("\n========== 评估结果 ==========")
    print(f"AOI: {args.AOI}")
    print(f"总帧数: {frames_processed}")
    print(f"总真值点数（移动）: {total_tp + total_fn}")
    print(f"总检测点数: {total_tp + total_fp}")
    print(f"TP: {total_tp}, FP: {total_fp}, FN: {total_fn}")
    print(f"精确率 (Precision): {precision:.4f}")
    print(f"召回率 (Recall):    {recall:.4f}")
    print(f"F1 分数:            {f1:.4f}")

if __name__ == "__main__":
    main()