...
同时对多个 AOI 进行推理，使用原始回归模型（无注意力机制）。
其他模块：原始分类CNN、光流增强背景减除、场景先验后处理。
每个 AOI 输出：
    - metrics.txt
    - CSV/（每帧检测经纬度）
    - visualizations/（每帧检测与真值叠加图像）
根目录输出：
    - summary.txt（总指标）
用法：
    python WAMI_detector_multi_AOI_origreg.py --png_root "D:/AOI_sequences" --truth_csv "path/to/truth.csv" --output_base "./results_orig_reg"
"""
...
def process_one_aoi(aoi, png_folder, out_dir, params, models):
    ...
    return precision, recall, f1, total_gt, total_det, total_tp, total_fp, total_fn


def main():
    parser = argparse.ArgumentParser(description='Multi-AOI WAMI Detector - Original Regression Model')
    ...
    parser.add_argument('--binary_model_dir', default='Models/')   # 原始分类与回归模型均从此目录加载
    ...
    parser.add_argument('--prior_threshold', type=float, default=0.31)
    ...
    print("加载原始分类与回归模型（无注意力）...")
    import TrainNetwork.BaseFunctions as bf
    # 一次性加载两个原始模型
    model_bin, aveImg_bin, model_reg_orig, aveImg_reg_orig = bf.ReadModels(args.binary_model_dir)
    ...
    models = (model_bin, aveImg_bin, model_reg_orig, aveImg_reg_orig)
    ...
    for aoi in args.aoi_list:
        ...
        print(f"\n#### 处理 AOI {aoi} ####")
        p, r, f1, gt, det, tp, fp, fn = process_one_aoi(
    ...
    with open(summary_path, 'w') as f:
        f.write("========== Multi-AOI Evaluation Summary (Original Regression) ==========\n")
        for aoi in args.aoi_list:
